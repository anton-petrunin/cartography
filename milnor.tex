...
If $\balance_\rho K\le 0$, then the coastline of $K$ is nondegenerate; in other words, $K$ has two distinct corners, say $l$ and~$r$.
Furthermore, $\phi+\psi\le \pi$, where $\phi$ and $\psi$ denote the angles that the chord $[l,r]$ makes with the borderline at $l$ and $r$, respectively.
\end{thm}
...
\parit{Proof.} Note that $f\:K\to \tilde K$ is Lipschitz.
...
Let $\vec i(s)$ be a normal Jacobi field along the chord induced by the foliation; after rescaling, we can assume that
...
Applying the nonpositive-balance condition to an infinitesimal slice, we get
...
\]
Note that $0\le a\leq a+s\le \tfrac\pi2$; the last inequality follows from \ref{lem:nondegenerate}.
...
Since  $0\le a\leq a+s\le \tfrac\pi2$, we have $\sin s\leq s$ and $\mu\cdot\cos (a+s)\geq 0$.
Hence
