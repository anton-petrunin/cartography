...
Furthermore, applying the spherical area formula (which is a special case of the Gauss--Bonnet theorem) to the complement $K\setminus L$, we get
...
Applying the spherical area formula as in the proof above, we also obtain the following.

\begin{thm}{Lemma}\label{lem:nondegenerate}
Let $K$ be an admissible set.
If $\balance_\rho K\le 0$, then the coastline of $K$ is nondegenerate.
\end{thm}
...
\]
Note that $0\le a\leq a+s\le \tfrac\pi2$; furthermore, by $({*}{*})$, we have $\mu\le 1-\cos\rho$.
...
Let $(M,g)$ be a noncomplete simply connected Riemannian 2-manifold with constant curvature $1$ and area equal to $\area D_\rho$.
