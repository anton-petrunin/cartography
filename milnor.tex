...
Note that $S$ is a sector in the $\rho$-disk.
...
As we mentioned already, a straightforward limiting  procedure provides a map $f_0\:\Omega\to\RR^2$ with optimal Lipschitz constants;
this is also stated in Milnor's paper.
...
\begin{enumerate}[(a)]
\item There is a closed subset $X$ of $\Omega$ such that the differential $d_xf_0$ is an isometry at every $x\in X$, and the complement $\Omega\setminus X$ has a foliation by geodesics such that $f_0$ isometrically maps each geodesic in the foliation to a geodesic in the plane.
...
The following question addresses a related problem for nonconvex domains.
