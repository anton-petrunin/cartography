...
Furthermore, by $({*}{*})$, we have $\mu\le 1-\cos\rho$.

For fixed $a$, the quantity $\nu$ is nondecreasing as a function of $s$.
The computations below show that for fixed $\mu$, the quantity $\nu$ is nonincreasing as a function of $a$.
Thus $\nu$ is maximized when $a=0$ and $s=\rho$, where $\nu=\tfrac{\rho}{\sin\rho}$; hence $({*})$ follows.

\parit{Computations.}
Fix $\mu$; regard $s$ and $\nu$ as functions of $a$.
Differentiating $\mu$, we get $(a+s)'=\tfrac{\sin a}{\sin(a+s)}$.
