...
\begin{document}
\title{\thetitle\\ (preliminary version)}
\author{\theauthors}
\date{}
\maketitle
...
for some constants $L_1$ and $L_2$ and any two points $x,y\in \Omega$;
here $|\ -\ |_{\mathbb{S}^2}$ and  $|\ -\ |_{\RR^2}$ stand for the geodesic distance in the sphere $\mathbb{S}^2$ and the Euclidean plane $\RR^2$, respectively.

We will be interested in upper bounds on the distortion ratio $L_2/L_1$ for a given set $\Omega$.
In other words, given a set $\Omega$, we want to construct its cartographic projection with small distortion.
After rescaling, we may assume that $L_1=1$ (so, $f$ is noncontracting), and we have to minimize $L_2$.

Let us denote by $D_\rho$ the round disc of radius $\rho$ in $\mathbb{S}^2$.
...
John Milnor \cite{milnor} observed that this value is optimal;
that is, \textit{if $f\:D_\rho\to\RR^2$ is a bi-Lipschitz map,
then its distortion ratio must be at least $\tfrac{\rho}{\sin \rho}$; moreover, if the equality holds, then, up to rescaling, the map $f$ is the same as above.}
In the same paper he formulated the following problem.
...
We impose a certain balancing property on the subdivision, which makes it possible to keep the distortion under control.

Our solution was found while writing a note \cite{petrunin2026} for school students on Gary Lawlor's proof of the isoperimetric inequality \cite{lawlor}.
...
A convex subset $K\subset \Omega$ is called \emph{admissible} if it is bounded by an arc of $\partial \Omega$ and a polygonal line $a_0a_1\dots a_n$ joining the endpoints of this arc.
The arc $\delta=\partial \Omega\cap K$ will be called the \emph{coastline} of $K$;
the polygonal $a_0a_1\dots a_n$ will be called the \emph{borderline} of $K$;
...
Note that every sector of $D_\rho$ is a $\rho$-balanced set.
...
in particular $S$ is $\rho$-balanced.
Furthermore, $\turn S=\turn L$, and since $S\subset L$, we have $\area S\le \area L$.
Therefore,
\[0=\balance_\rho S\le \balance_\rho L;\]
moreover, in the case of equality, we have $L=S$.
...
Let $K$ be an admissible set and $S$ be a sector of a disc in the sphere.
We say that $K$ is \emph{properly inscribed} in $S$ if the corners of $K$ lie on two different sides of $S$ and the coastline of $K$ cuts off from $S$ a convex set that contains the center of $S$.
...
If $\balance_\rho K=0$, then we have equalities both times, and hence $S=L\z=K$.
\qeds
...
Observe that each $K_i$ is $\rho$-balanced.
...
Similarly, if $p$ and $q$ are close to $r$, then by \ref{lem:angle}, $\balance_\rho R\le 0$, and $\balance_\rho L\ge0$.
Since $\balance_\rho L$ depends continuously on the choice of the chord, we can choose a chord $[p,q]$ that splits $K$ into two $\rho$-balanced sets.

Furthermore, \ref{lem:angle} implies that for any $\eps>0$ we can choose $\delta>0$ such that if the coastline of $K$ is longer than $\eps$,
then the coastline of each slice $L$ and $R$ is shorter than that of $K$ by at least $\delta$.
Therefore, repeating this construction recursively provides the needed subdivision.
\qeds
...
Recall that $\rho\le \tfrac\pi2$.
By the condition on chord length, the function
...
If $K'\ne K$ then the foliation has a chord with end at the right corner.
...
Suppose that the original foliation is \emph{spreading}; that is, every slice between two chord of the foliation, or on one side of a chord, has turn at most~$\pi$.
Then the obtained half-lines do not intersect each other and they foliate a region in the interior of $\tilde K$.
\begin{figure}[h!]
\centering
\vskip-0mm
...
Further, choose an oriented orthonormal frame $\tilde {\vec v}$, $\tilde {\vec w}$ in the interior of $\tilde K$ such that $\tilde{\vec v}$ points in the direction of the half-line foliation.
...
Since the slice between chords has nonpositive balance,
differentiating, we get
\[\lambda(\rho)\cdot(\cos a- \cos (a+s))=\lambda(\rho)\cdot\int\limits_0^{s}\sin (a+t)\cdot dt\le\cos a.\]

Let $0\le a\leq a+s\le \tfrac\pi2$, and set
\[
x=\cos a-\cos (a+s),
\qquad
y=\frac{\sin a+s\cdot \cos a}{\sin (a+s)}.
\]
By straightforward computations, we get the following.
\begin{enumerate}[(i)]
\item for fixed $x$, the quantity $y$ is nonincreasing as a function of $a$,
\item for fixed $a$, the quantity $y$ is nondecreasing as a function of $s$.
\end{enumerate}
Hence $({*})$ follows.
%Fix $x$ and regard $b$ as a function of $a$.
%Differentiating $\cos a-\cos b=x$, we obtain
%\[b'=\frac{\sin a}{\sin b}.\]
%A direct computation gives
%\[y'=\frac{\sin a}{\sin^3 b}\left[\sin(b-a)-(b-a)\cdot (1+x\cdot\cos b)\right].\]
%Since
%\[\sin(b-a)\leq b-a\qquad\text{and}\qquadx\cdot\cos b\geq 0,\]
%we have $y'\leq0$.
%If $x>0$ and $a>0$, the inequality is strict.
\qeds
...
The additional half-lines in the foliation of $\tilde K$ are mapped to  geodesics making the same angle with $\beta$ that start from the corners of $K$.
...
\[\tilde {\vec i}(s)=\tilde {\vec w}(s)\cdot (\sin a+ s\cos a)\]
...
Consider the development $T$;
it is a geodesic tree in the plane with the same edge lengths and the same oriented angles as in $T$.
Extend the edges of the development beyond their endvertices; denote the resulting tree by $\tilde T$.
...
Suppose $\eps$ is very small; in particular, $\rho+\eps<\tfrac\pi2$.
...
Since $\eps>0$ can be chosen arbitrarily, Arzelà--Ascoli theorem provides a map $f_0\:\Omega\to\RR^2$ with Lipschitz constants $1$ and $\tfrac\rho{\sin(\rho)}$.
\qeds
...
As we mentioned already, a straightforward limit procedure provides a map $f_0\:\Omega\to\RR^2$ with optimal Lipschitz constants;
this was also stated in John Milnor's paper.
We can assume that $f_0$ has lower Lipschitz constant 1; therefore the upper Lipschitz constant $L_2$ is minimal.
In this case we expect that $f_0$ has the following properties:
\begin{enumerate}[(a)]
\item There is a subset $X$ of $\Omega$ such that the differential $d_xf_0$ is an isometry for any $x\in X$, and the complement $\Omega\setminus X$ has a foliation by geodesics such that $f_0$ isometrically maps each geodesic in the foliation to a geodesic in the plane.
\item The local upper Lipschitz constants approach $L_2$ at the boundary of $\Omega$.
\item If $\Omega$ is convex then so is its image $f_0(\Omega)$.
\end{enumerate}


\begin{thm}{Question}
Let $(M,g)$ be a simply connected (noncomplete) Riemannian 2-manifold with constant curvature $1$ and area equal to $\area D_\rho$.
Is there a homeomorphism from $(M,g)$ to a flat 2-manifold with distortion ratio at most $\tfrac \rho{\sin \rho}$?
\end{thm}
